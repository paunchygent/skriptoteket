...
function speakerOverlayCoverageComplete(
  entries: readonly ConversionHubTranscriptSpeakerOverlayEntry[],
  ...
  return labels.every((label) => namedLabels.has(label));
}
...
<template>
  <div
    class="@container flex min-h-0 w-full flex-1 overflow-hidden bg-canvas"
    data-test="transcript-result-surface"
  >
    <div
      class="grid min-h-0 w-full flex-1 gap-0 @min-[56rem]:grid-cols-[minmax(0,1fr)_minmax(18rem,21rem)]"
      data-test="transcript-result-grid"
    >
      <section
      ...
      <aside
        class="grid min-h-0 min-w-0 content-start gap-5 border-x border-b border-navy/20 bg-panel-muted px-4 py-4 @min-[56rem]:border-l-0 @min-[56rem]:border-t"
      ...
      >
        ...
        >
          ...
          <div class="min-w-0">
            <p
              class="inline-flex min-h-8 min-w-0 items-center gap-2 text-xs font-medium leading-snug text-navy/65"
              data-test="transcript-speaker-overlay-state"
              aria-live="polite"
            >
              <template v-if="speakerOverlayStatus === 'loading'">Hämtar namn.</template>
              <template v-else-if="speakerOverlayStatus === 'saving'">
                Sparar namn automatiskt.
              </template>
              <template v-else-if="speakerOverlayStatus === 'saved'">
                <Check
                  class="h-4 w-4 shrink-0 text-success"
                  aria-hidden="true"
                />
                <span>Namnen är sparade.</span>
              </template>
              ...
              <template
                v-else-if="
                  speakerOverlayCoverageComplete(speakerOverlayEntries, speakerLabels(transcript))
                "
              >
                Namnen sparas automatiskt.
              </template>
              <template v-else>
                Fyll i namn för alla talare. Namn sparas automatiskt.
              </template>
            </p>
          </div>
        </section>
