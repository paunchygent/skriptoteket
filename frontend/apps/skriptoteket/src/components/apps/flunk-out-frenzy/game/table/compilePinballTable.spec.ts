/**
 * compilePinballTable regression tests for donor-backed device geometry.
 *
 * These checks keep authored pinball-table semantics honest by asserting that
 * donor-provided sensor rotation survives compilation instead of collapsing
 * back to axis-aligned collider plans.
 */
...
describe("compilePinballTable", () => {
  ...
  it("carries the missing donor shooter corridor walls into the compiled solids plan", () => {
    ...
    expect(upperHandoff).toMatchObject({
      ...
      shape: {
        kind: "convex-polygon",
        vertices: VPW_SHOOTER_HANDOFF_UPPER_POLYGON,
      },
      sensor: false,
    });
    expect(lowerHandoff).toMatchObject({
      ...
      shape: {
        kind: "convex-polygon",
        vertices: VPW_SHOOTER_HANDOFF_LOWER_POLYGON,
      },
      sensor: false,
    });
  });
  ...
  it("pins the retained Wall263 shoulder donor points and keeps removed tail ownership explicit", () => {
    ...
    expect(PROTOTYPE_ALPHA_TABLE_SPEC.solids.map((solid) => solid.id)).toEqual(
      expect.arrayContaining([
        "shooter-handoff-upper",
        "shooter-handoff-lower",
        "apron-1",
        "apron-2",
      ]),
    );
    ...
    expect(compiled.physics.colliders).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          shape: {
            kind: "convex-polygon",
            vertices: VPW_SHOOTER_OUTER_POLYGON,
          },
          sensor: false,
        }),
        expect.objectContaining({
        ...
        expect.objectContaining({
          ...
          shape: {
            kind: "convex-polygon",
            vertices: VPW_SHOOTER_DIVIDER_POLYGON,
          },
          sensor: false,
        }),
        expect.objectContaining({
  ...
  it("keeps elevated donor receive-mouth walls visible but out of playfield colliders", () => {
    ...
    expect(compiled.render.nodes).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          id: "right-receive-mouth-outer:render",
          kind: "polygon",
        }),
        expect.objectContaining({
          id: "right-receive-mouth-inner:render",
          kind: "polygon",
        }),
      ]),
    );
...
function expectWireRolloverShape(
