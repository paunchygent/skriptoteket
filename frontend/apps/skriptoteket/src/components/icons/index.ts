/**
 * Shared icon wrapper registry.
 *
 * Relationships:
 * - exports semantic icon wrappers for planner and site/app control surfaces
 * - keeps runtime components behind code-facing names from the symbol contract
 */

export { default as IconAdjustments } from "./IconAdjustments.vue";
export { default as IconAi } from "./IconAi.vue";
export { default as IconArrow } from "./IconArrow.vue";
export { default as IconArmchair } from "./IconArmchair.vue";
export { default as IconBan } from "./IconBan.vue";
export { default as IconBookmark } from "./IconBookmark.vue";
export { default as IconCatalog } from "./IconCatalog.vue";
export { default as IconCheck } from "./IconCheck.vue";
export { default as IconClassList } from "./IconClassList.vue";
export { default as IconClassroom } from "./IconClassroom.vue";
export { default as IconClipboardList } from "./IconClipboardList.vue";
export { default as IconCode } from "./IconCode.vue";
export { default as IconCopy } from "./IconCopy.vue";
export { default as IconDebug } from "./IconDebug.vue";
export { default as IconDownload } from "./IconDownload.vue";
export { default as IconEdit } from "./IconEdit.vue";
export { default as IconFileAudio } from "./IconFileAudio.vue";
export { default as IconFileSpreadsheet } from "./IconFileSpreadsheet.vue";
export { default as IconFileText } from "./IconFileText.vue";
export { default as IconFitView } from "./IconFitView.vue";
export { default as IconFirstPage } from "./IconFirstPage.vue";
export { default as IconGraduationCap } from "./IconGraduationCap.vue";
export { default as IconGroupsWorkspace } from "./IconGroupsWorkspace.vue";
export { default as IconHelp } from "./IconHelp.vue";
export { default as IconHistory } from "./IconHistory.vue";
export { default as IconImageAsset } from "./IconImageAsset.vue";
export { default as IconInfo } from "./IconInfo.vue";
export { default as IconKeepApart } from "./IconKeepApart.vue";
export { default as IconKeepNear } from "./IconKeepNear.vue";
export { default as IconLastPage } from "./IconLastPage.vue";
export { default as IconLink2 } from "./IconLink2.vue";
export { default as IconLock } from "./IconLock.vue";
export { default as IconMinus } from "./IconMinus.vue";
export { default as IconMoreVertical } from "./IconMoreVertical.vue";
export { default as IconNextPage } from "./IconNextPage.vue";
export { default as IconOverview } from "./IconOverview.vue";
export { default as IconPlus } from "./IconPlus.vue";
export { default as IconPreview } from "./IconPreview.vue";
export { default as IconPreviousPage } from "./IconPreviousPage.vue";
export { default as IconProfile } from "./IconProfile.vue";
export { default as IconPresentation } from "./IconPresentation.vue";
export { default as IconRefresh } from "./IconRefresh.vue";
export { default as IconRedo } from "./IconRedo.vue";
export { default as IconRole } from "./IconRole.vue";
export { default as IconRules } from "./IconRules.vue";
export { default as IconSave } from "./IconSave.vue";
export { default as IconSchool } from "./IconSchool.vue";
export { default as IconSearch } from "./IconSearch.vue";
export { default as IconSeat } from "./IconSeat.vue";
export { default as IconSeatingPlan } from "./IconSeatingPlan.vue";
export { default as IconSeparatePdfs } from "./IconSeparatePdfs.vue";
export { default as IconSettings } from "./IconSettings.vue";
export { default as IconShuffle } from "./IconShuffle.vue";
export { default as IconSpinner } from "./IconSpinner.vue";
export { default as IconStudents } from "./IconStudents.vue";
export { default as IconRun } from "./IconRun.vue";
export { default as IconRunHistory } from "./IconRunHistory.vue";
export { default as IconTeacherAnchor } from "./IconTeacherAnchor.vue";
export { default as IconTrash } from "./IconTrash.vue";
export { default as IconUndo } from "./IconUndo.vue";
export { default as IconUsersRound } from "./IconUsersRound.vue";
export { default as IconVaultFiles } from "./IconVaultFiles.vue";
export { default as IconWarning } from "./IconWarning.vue";
export { default as IconX } from "./IconX.vue";
export { default as IconZoomIn } from "./IconZoomIn.vue";
export { default as IconZoomOut } from "./IconZoomOut.vue";
export { default as IconCombinePdf } from "./IconCombinePdf.vue";
