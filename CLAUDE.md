...
- Always load: `skriptoteket-devops` for Hemma/deploy/compose/env; `skriptoteket-local-devops` for local development; `skriptoteket-frontend-specialist` (and `brutalist-academic-ui` for styling) for SPA work; and global `observability-stack` for Grafana/Prometheus/Loki/Jaeger/structlog.
