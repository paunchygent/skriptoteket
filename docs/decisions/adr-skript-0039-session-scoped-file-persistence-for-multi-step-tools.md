...
Source: `docs/adr/adr-0039-session-file-persistence.md`. Session-scoped file persistence for multi-step tools.

Multi-step tools (using `next_actions`) run each step in an isolated container. Currently, uploaded files are only available in the initial run - action runs receive only the action payload (with `{action_id, input, state}`). This creates a design conflict: - `tool_sessions.state` (JSON) is intended for workflow metadata + user preferences, not file content - Tools that need prior input files across steps must either: - Store file content in JSON state (hacky, bloated, violates intended semantics) - Require user to re-upload on each action (poor UX) Neither option is architecturally correct. The proper solution is session-scoped file storage that carries uploaded files through action runs wi
