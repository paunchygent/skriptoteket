...
Tool execution currently happens synchronously within the API request path. ADR-SKRIPT-0016 enforces a global concurrency cap
...
We need a durable, observable execution queue and worker loop while keeping the existing runner contract (ADR-SKRIPT-0015) and
Docker runner constraints (ADR-SKRIPT-0013). ADR-SKRIPT-0007 also defers Kafka/streaming infrastructure until needed, so a Postgres
queue is preferred.
...
### 7) Keep ADR-SKRIPT-0016 behavior as a fallback mode

Queueing is enabled behind settings (e.g., `RUNNER_QUEUE_ENABLED`). When disabled, the system behaves exactly as ADR-SKRIPT-0016
(cap + reject, no queue). When enabled, ADR-SKRIPT-0016 is superseded for production execution but remains the fallback policy
