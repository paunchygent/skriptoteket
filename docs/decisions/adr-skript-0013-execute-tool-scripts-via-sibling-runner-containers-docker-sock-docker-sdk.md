...
Source: `docs/adr/adr-0013-execution-ephemeral-docker.md`. Execute tool scripts via sibling runner containers (docker.sock + Docker SDK).

The Skriptoteket application itself runs in a Docker container. It needs to spawn *ephemeral* containers to execute untrusted/admin-authored tool scripts safely. Binding host paths (`-v /host/path:/container/path`) is unreliable because the app container does not know the host's filesystem layout. Mounting `docker.sock` is powerful: if the app is compromised, an attacker may control containers on the host. **Sibling Container Model (Docker-outside-of-Docker):** 1.  **Socket Access:** The app container mounts `/var/run/docker.sock`. 2.  **I/O Strategy:** *   **Input:** App injects the script + input file via Docker Engine archive copy APIs (`put_archive`) into `/work` (a per-run Docker volume
