...
Source: `docs/adr/adr-0047-layout-editor-v1.md`. Layout editor v1 (typed layout output + platform renderer).

Some tools (notably seating/room planning) need an interactive UI where a user can: - see a “room layout” with physical objects (desks, door, window, whiteboard) - assign “tokens” (students) to “slots” (seats) - iterate, fine-tune, and then finalize to an artifact + a JSON representation - return later and continue from remembered settings/previous arrangements The current contract supports: - typed outputs (markdown/table/json/html_sandboxed) and typed action forms - persisted `state` and session turn-taking (`start_action`) But it does **not** support tool-provided interactive UI JavaScript (by design). `html_sandboxed` renders in a fully sandboxed iframe (no scripts), so it cannot be used
