...
The Tool UI contract v2 (ADR-SKRIPT-0022) supports multi-step tools via:

- `ui_payload.next_actions[]` (platform-rendered action forms)
- persisted session `state` with optimistic concurrency (`state_rev`) (ADR-SKRIPT-0024)
...
We will persist normalized tool state for sandbox runs using the existing `tool_sessions` model (ADR-SKRIPT-0024).
...
(ST-14-06 / ADR-SKRIPT-0044) is implemented, sandbox sessions and actions will move to
...
- Tools that need prior input files across steps rely on session-scoped file persistence (ADR-SKRIPT-0039). Sandbox action runs
  receive the original uploaded files in `/work/input/` (same as production), while the action payload is provided via
  the request envelope (ADR-SKRIPT-0063).
