---
type: reference
id: REF-SKRIPT-GENERAL-reference-curated-app-reagent-prep-chef
...
retired_ids:
- REF-curated-app-reagent-prep-chef
summary: 'Reference: Curated app — Reagent Prep Chef'
---

## Overview

Source: `docs/reference/ref-curated-app-reagent-prep-chef.md`. Reference: Curated app — Reagent Prep Chef.

This document adapts **“The Reagent Prep Chef”** into a **Skriptoteket Curated App** (ADR-0023) implemented in the FastAPI backend, rendered through **Tool UI contract v2** (ADR-0022), and persisted via **tool sessions + ui_payload** (ADR-0024). The goal is a teacher-first, SOTA-feeling prep calculator without “scripts fighting the UI”: the platform renders a typed form and a structured prep sheet, while the backend stays deterministic and audit-friendly. - This app **does not** replace an SDS, local lab rules, or supervision requirements. - The app **must not** “guess” hazards. It only returns **curated** safety entries for known reagents and otherwise returns a **Consult SDS** response. -
