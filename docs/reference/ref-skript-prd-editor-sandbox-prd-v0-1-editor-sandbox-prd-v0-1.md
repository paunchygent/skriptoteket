---
type: reference
id: REF-SKRIPT-PRD-editor-sandbox-prd-v0-1
...
status: active
reference_kind: prd
summary: Editor sandbox PRD v0.1
---

## Product Outcome And Users

Source: `docs/prd/prd-editor-sandbox-v0.1.md`. Editor sandbox PRD v0.1.

Provide a safe, fast iteration workflow for tool authors inside the editor sandbox: preview unsaved code and schemas, test multi-step actions, isolate sandbox settings from production, and prevent concurrent draft edits with explicit locks and admin override. - Authors can run unsaved editor changes without publishing or saving drafts. - Multi-step tools can be tested end-to-end using snapshot-stable next_actions. - Sandbox settings are isolated per user and draft head and never leak into published runs. - Draft head edits and sandbox operations are protected by explicit locks with TTL heartbeat and admin/superuser takeover. - Sandbox input and settings forms match the production run UI beha

## Capability Direction

The source PRD direction is retained without adding product scope.

## Boundaries And Non-Goals

No adjacent product behavior is authorized by this reference.

## Success Signals

Use the source success and acceptance signals as evidence for later backlog work.

## Governed Follow-Up

Implementation authority remains in the linked backlog/ADR records.
