...
|----------|-----------|----------|
| New epic instead of extending EPIC-26 or EPIC-27 | Export/import and smart-assignment are active but narrower lanes; the redesign needs its own outcome and review surface | [x] |
| Desktop-first is the canonical product stance | Prevent desktop workspaces from collapsing into mobile-first stacked compromises | [x] |
| Symbol system ships before layout restyling | Shared operation language must stabilize before workspace-specific redesign begins | [x] |
| Shared primitives ship before workspace-specific slices | Later stories should build on common shell/zoning/scroll/control seams | [x] |
| Reduced mobile companion layouts ship last | Smaller-screen behavior should adapt from the settled desktop product, not shape it prematurely | [x] |
...
- [x] Sequencing reduces redesign risk instead of front-loading visual churn
- [x] Risks and overlaps with existing EPIC-26 stories are identified clearly
...
Initial epic review opened with `changes_requested`; the required planning changes listed below were
completed and the epic review is now approved overall, with later implementation gates handled in
the dedicated `REV-PR-*` follow-up records.
...
- [Review: PR-0226 shared planner shell parity and grouping viewport-height stabilization](review-pr-0226-shared-planner-shell-parity-and-grouping-viewport-height-stabilization.md)
- [Review: PR-0227 exact two-row grouping board height contract at desktop baseline](review-pr-0227-exact-two-row-grouping-board-height-contract-at-desktop-baseline.md)
- [Review: PR-0228 planner workspace shell, breakpoint, and overflow contract](review-pr-0228-planner-workspace-shell-breakpoint-and-overflow-contract.md)
- [Review: PR-0229 planner toolbar breakpoint overflow escalation and undo/redo shortcut parity](review-pr-0229-planner-toolbar-breakpoint-overflow-escalation-and-undo-redo-shortcut-parity.md)

New PR and story follow-up review gates should live in their own target-based review docs rather than as supplemental sections inside this epic review record.
