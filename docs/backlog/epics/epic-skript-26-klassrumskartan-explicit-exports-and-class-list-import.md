...
closeout_review:
  record: inline
  status: not_started
outcome: Teachers can export Klassrumskartan seating plans as a poster-grade standalone
  PDF, import class lists from common teacher files with confirmation before save,
  export seating as editable XLSX, export grouping first as an editable XLSX collaboration
  artifact and then as an A4 portrait PDF presentation artifact, publish immutable
  shareable HTML/CSS export links for grouping and seating with reliable renderer-derived
  Teams/social previews, and rely on teacher-facing planner surfaces that remain usable
...
- Keep public guest share persistence inside the accepted `ADR-SKRIPT-0084` exception:
...
### Source: Implementation Summary (as of 2026-03-26)
...
- Seating exports now have an explicit prepare-contract seam plus a fully local
  ...
  orchestration.
- `ADR-SKRIPT-0075` is now reflected in the shipped seating export lane, so
...
- Grouping exports now include a local default `XLSX` lane that presents a
  ...
  `A4` portrait.
- `ST-SKRIPT-26-06` adds the next governed export variant: immutable shareable
  HTML/CSS links for grouping and seating, with authenticated durable shares
  first (`TASK-SKRIPT-26-06-01`) and public guest shares with 60-day TTL and browser-held
  supersede/revoke behavior second (`PR-0273`). The 2026-04-30 UI assessment
  added two follow-up slices: `TASK-SKRIPT-26-06-02` replaces detached share panels with
  ...
  Microsoft Teams; `ST-SKRIPT-26-07`/`TASK-SKRIPT-26-07-01` turns that proof into the supported
  ...
  constraints. `TASK-SKRIPT-26-07-01` now stores 1200x630 PNG preview assets in PostgreSQL,
  ...
  2026-06-14 Hemma production build warning investigation created `TASK-SKRIPT-26-07-02` to
  ...
  `PR-0359` reviewed `TASK-SKRIPT-26-07-01` for stale-state repair on 2026-06-18 and left it
  ...
  unfurl proof are still outstanding, so `ST-SKRIPT-26-07` remains active.
...
- `ST-SKRIPT-26-08` is the follow-up visual parity lane for the actual downloaded PDF
...
- Adjacent desktop UI-hardening ideas that surfaced during export/import planning were later
  consolidated into `EPIC-SKRIPT-29`, which is now the canonical overhaul hub.
...
- Public guest share creation could weaken `ADR-SKRIPT-0079` unless it stays inside a
...
- For Klassrumskartan-owned artifacts, export rendering/runtime stays planner-owned inside
  Skriptoteket per `ADR-SKRIPT-0075`.
