...
starts: 2026-05-26
ends: 2026-06-08
objective: "Ship high-leverage interaction UX improvements for multi-step tools (sticky action inputs, progress, file references)."
prd: "PRD-tool-authoring-v0.1"
epics: ["EPIC-14"]
stories: ["ST-14-21", "ST-14-22"]
---
...
- Privacy: persisting action inputs must avoid accidentally storing sensitive values.
- Backwards compatibility: conventions must be opt-in (ignore unknown state keys).

## Execution plan (suggested)

1) Add “sticky inputs” behavior for next_actions (at minimum within the session; optionally per-device).
2) Define and document a minimal progress convention (e.g., `state.progress = { current, total, label? }`) and render it.
3) Expose a “Files available” panel derived from the known input manifest, including copyable canonical paths.

## Demo checklist

- Run an interactive tool with repeated actions and show that prior inputs are preserved/pre-filled.
- Show step progress for a 2–3 step tool (step indicator updates after each action).
- Upload 1–2 files and show the file reference UX (names + copy path) and that the tool can consume them.
