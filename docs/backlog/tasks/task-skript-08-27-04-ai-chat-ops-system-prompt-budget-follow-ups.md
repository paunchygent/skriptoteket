...
task_kind: story
acceptance_criteria:
- Edit-ops (Föreslå ändringar) no longer returns enabled=false due to chat-ops system
  prompt budget validation.
- LLM_CHAT_OPS_SYSTEM_PROMPT_MAX_TOKENS is high enough for editor_chat_ops_v1 under
  conservative token counting.
- A regression test composes chat/chat-ops prompts using the real TokenCounterResolver
  (not FakeTokenCounter).
