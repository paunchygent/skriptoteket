...
acceptance_criteria:
  - "Given a completed Flunk-Out Frenzy run, when the user submits a score, then the backend stores a lightweight score submission containing `app_id`, `app_version`, `ruleset_id`, owner user id, score value, and compact score summary metadata."
  - "Given the SPA requests the global leaderboard or the signed-in user's own leaderboard view, when the backend responds, then the response is a typed app-specific model scoped to the current `ruleset_id`."
  - "Given the current user has recently submitted a score, when they reload the game shell, then the app can show personal-best or latest-score state without leaking generic run mechanics into the competition UX."
