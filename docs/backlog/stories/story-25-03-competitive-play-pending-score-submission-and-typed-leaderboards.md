...
acceptance_criteria:
  - "Given a completed Flunk-Out Frenzy run, when the user submits a score, then the backend stores a pending submission containing `app_id`, `app_version`, `ruleset_id`, owner user id, score summary, and a replay asset reference or equivalent replay metadata."
