...
acceptance_criteria:
  ...
  - "Given the teacher authors `Keep apart`, `Keep near`, `Närmare läraren`, and `Use history` through the class-wide visual smart-rule surface, when smart seating runs, then those inputs affect the backend result without exposing raw weights, score tables, or rule-engine jargon."
