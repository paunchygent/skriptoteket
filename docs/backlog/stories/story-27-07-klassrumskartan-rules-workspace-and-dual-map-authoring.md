...
acceptance_criteria:
  ...
  - "Given one existing smart rule is shown in the rule summary, when the teacher chooses `Redigera`, then the correct tool is reactivated, the relevant students are preselected on the current map, and saving updates that rule rather than forcing remove-and-recreate."
