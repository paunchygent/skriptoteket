...
epic: "EPIC-32"
dependencies:
  ["ADR-0011", "ADR-0027", "ST-11-22", "ST-28-02", "ST-32-07", "ST-32-08", "ST-32-09"]
acceptance_criteria:
  ...
  - "Given the auth-entry page replaces the overloaded signed-out modal seam, when it renders, then it preserves an explicit intended post-login destination without depending on route-local modal state."
  - "Given Skriptoteket still needs launch-ready local auth now and HuleEdu SSO later, when this story ships, then the page-based auth-entry contract remains compatible with a future top-level HuleEdu-owned auth ceremony without reviving the old legacy `/login` page behavior."
