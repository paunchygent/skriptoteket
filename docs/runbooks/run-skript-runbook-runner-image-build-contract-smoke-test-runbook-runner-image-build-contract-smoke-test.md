---
type: runbook
id: RUN-SKRIPT-runbook-runner-image-build-contract-smoke-test
...
retired_ids:
- RUN-runner-image
summary: 'Runbook: Runner image build + contract smoke test'
system: skriptoteket-runner
---

## Trigger

The source does not provide a separate trigger section; no additional trigger is recorded.
...
The source does not provide a separate expected results section; no additional expected results is recorded.
...
### Source: When to use this runbook
...
### Source: 1) Build the runner image (local)

```bash

### Source: Build runner image with BuildKit via Compose

docker compose -f compose.prod.yaml --profile build-only build runner
```

### Source: 2) Build the runner image (home server / production)
...
### Source: 3) Smoke test the contract (local dev)
...
### Source: Tool runs fail to start runner containers
...
### Source: Settings are not applied in the runner
