...
Additional targets are tracked in `REF-frontend-transition-continuity-v1`.
...
- Review approval is required before `ADR-0077` moves from `proposed` to `accepted` and before the
