...
2.  **I/O Strategy:**
    *   **Input:** App injects the script + input file via Docker Engine archive copy APIs (`put_archive`) or uses a strictly scoped Docker volume.
    ...
    *   **Filesystem:** Runner root filesystem is read-only; only `/work` and `/tmp` are writable (tmpfs/volume).
