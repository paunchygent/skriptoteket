...
`ADR-0079` keeps curated apps fail-closed for public access unless each app
...
`ST-21-03` now separates two Conversion Hub product needs:
...
This ADR authorizes one narrow exception to `ADR-0079`:
...
- `ADR-0079` remains accepted and historically accurate, but its
