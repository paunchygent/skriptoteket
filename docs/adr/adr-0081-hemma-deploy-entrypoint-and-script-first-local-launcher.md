...
### 1. Keep the on-host script as the only deploy logic
...
### 2. Add one canonical local launcher
...
### 3. The local launcher must be detached, script-first, and quoting-safe
...
### 4. Operator docs should treat the launcher as the canonical local entrypoint
