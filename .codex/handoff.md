...
- Date: 2026-06-30.
- Branch: `main`.
- Latest closed work: Story 58 correction replay download/save production failure is fixed through HuleEdu TASK-0831 plus retained Dev/Prod Skriptoteket proof. Current production proof passed at `.artifacts/playwright-pr-0337-correction-session-live/20260630T110339Z/manifest.json`; current Dev proof passed at `.artifacts/playwright-pr-0337-correction-session-live/20260630T111643Z/manifest.json`. Both use the real `ak7_lag_och_ratt_with_image.dxe`, retain `service-monitoring.json` plus service logs, and prove replay PDF/ZIP download `200` plus Save to My Files `200`.
...
- `PR-0410` is implemented and approved by `REV-PR-0410`: current live proof has production `.artifacts/playwright-pr-0337-correction-session-live/20260630T110339Z/manifest.json` and Dev `.artifacts/playwright-pr-0337-correction-session-live/20260630T111643Z/manifest.json`; both prove replay PDF/ZIP download `200`, Save to My Files `200`, content inspection, and retained service logs. Side findings: production exact-name preserve-source rerun at `.artifacts/playwright-pr-0337-correction-session-live/20260630T110042Z/manifest.json` strict-replayed an existing ready session and hit correction-session `409`; first Dev attempt at `.artifacts/playwright-pr-0337-correction-session-live/20260630T111335Z/manifest.json` hit a transient Gateway connect timeout before the green rerun. This closes PR-0410 incident proof only; Sir Convert Story 58 still needs broader story-level live replay proof.
- Story 58 stale replay proof is mechanically reconstructed for unconsumed prod records, but live Gateway replay requires the owner-scoped private inputs (`STORY58_IDEMPOTENCY_KEY` and `STORY58_OWNER_SCOPE` or equivalent private files). Smoke-account proof at `.artifacts/story-58-gateway-stale-replay/20260630T033858Z/manifest.redacted.json` correctly returned `fresh_admission`, so it is not final stale-replay proof.
