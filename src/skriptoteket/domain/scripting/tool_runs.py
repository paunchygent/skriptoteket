...
class ToolRun(BaseModel):
    ...
    @model_validator(mode="after")
    def _validate_timestamps(self) -> "ToolRun":
        ...
        if self.status in {
            ...
            if self.started_at is None:
                raise ValueError(f"status={self.status.value} requires started_at")
        if self.status in {
            RunStatus.SUCCEEDED,
            RunStatus.FAILED,
            RunStatus.TIMED_OUT,
            ...
            if self.finished_at is None:
                raise ValueError(f"status={self.status.value} requires finished_at")
...
def finish_run(
    *,
    ...
    if run.started_at is None:
        raise DomainError(
            code=ErrorCode.CONFLICT,
            message="Only started runs can be finished",
            details={"status": run.status.value},
        )
...
def start_queued_run(
    *,
    ...
    if run.status is not RunStatus.QUEUED:
        raise DomainError(
            code=ErrorCode.CONFLICT,
            message="Only queued runs can be started",
            details={"status": run.status.value},
        )
    if run.started_at is not None:
        raise DomainError(
            code=ErrorCode.CONFLICT,
            message="Run is already started",
            details={"status": run.status.value},
        )
    if now < run.requested_at:
        raise validation_error(
            "started_at cannot be before requested_at",
            details={"requested_at": run.requested_at.isoformat(), "started_at": now.isoformat()},
        )
...
def requeue_running_run(
    *,
    ...
    if run.status is not RunStatus.RUNNING:
        raise DomainError(
            code=ErrorCode.CONFLICT,
            message="Only running runs can be requeued",
            details={"status": run.status.value},
        )
    if run.finished_at is not None:
        raise DomainError(
            code=ErrorCode.CONFLICT,
            message="Cannot requeue a finished run",
            details={"status": run.status.value},
        )
    if run.started_at is not None and now < run.started_at:
        raise validation_error(
            "now cannot be before started_at",
            details={"started_at": run.started_at.isoformat(), "now": now.isoformat()},
        )
