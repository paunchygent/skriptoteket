...
class InMemoryPublicHelperRequestThrottle(PublicHelperThrottleProtocol):
    ...
    def evaluate_request(
        ...
        *,
        ...
        self._prune(request_times=request_times, now=now)
        if len(request_times) < self._max_requests:
            return PublicHelperThrottleDecision(is_rate_limited=False)

        oldest_request = request_times[0]
        retry_after = math.ceil((oldest_request + self._window - now).total_seconds())
    ...
    def _prune(self, *, request_times: deque[datetime], now: datetime) -> None:
