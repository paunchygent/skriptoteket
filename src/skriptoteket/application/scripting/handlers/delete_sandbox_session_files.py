...
class DeleteSandboxSessionFilesHandler(DeleteSandboxSessionFilesHandlerProtocol):
    def __init__(
        self,
        *,
    ...
    async def handle(
        ...
        *,
        ...
        deleted = await self._session_files.delete_files(
            tool_id=version.tool_id,
            user_id=actor.id,
            context=_sandbox_files_context(command.version_id),
            names=command.names,
        )

        return DeleteSandboxSessionFilesResult(
            tool_id=version.tool_id,
