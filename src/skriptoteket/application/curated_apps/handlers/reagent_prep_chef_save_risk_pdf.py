...
class ReagentPrepChefSaveRiskPdfHandler(ReagentPrepChefSaveRiskPdfHandlerProtocol):
    def __init__(
        self,
        *,
    ...
    async def handle(
        self, *, actor: User, command: ReagentPrepChefRiskAssessmentRequest
    ) -> ReagentPrepChefSavePdfResult:
        pdf_bytes = await self._export_pdf.handle(actor=actor, command=command)
