...
from skriptoteket.application.favorites.commands import AddFavoriteCommand, FavoriteStatusResult
...
class AddFavoriteHandler(AddFavoriteHandlerProtocol):
    def __init__(
        self,
        *,
    ...
    async def handle(self, *, actor: User, command: AddFavoriteCommand) -> FavoriteStatusResult:
        ...
        if tool is not None and tool.is_published:
            async with self._uow:
                await self._favorites.add_tool(user_id=actor.id, tool_id=tool.id)
            return FavoriteStatusResult(id=tool.id, is_favorite=True)
        ...
        if app is not None and has_at_least_role(user=actor, role=app.min_role):
            async with self._uow:
                await self._favorites.add_app(user_id=actor.id, app_id=app.app_id)
            return FavoriteStatusResult(id=app.tool_id, is_favorite=True)
