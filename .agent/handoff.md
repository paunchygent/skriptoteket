...
- Backend now: ST-11-03/04/05/06 done; ST-11-07 in progress (tool run API)
- Frontend now: ST-11-01/02/03/06 done; ST-11-07 in progress (tool run + results views)
...
- ST-11-06 Phase 2 (frontend views): Created three Vue browse views consuming the catalog API. Routes: `/browse` (BrowseProfessionsView), `/browse/:profession` (BrowseCategoriesView), `/browse/:profession/:category` (BrowseToolsView). All routes `requiresAuth: true`. Files: `frontend/apps/skriptoteket/src/router/routes.ts`, `frontend/apps/skriptoteket/src/views/BrowseProfessionsView.vue`, `BrowseCategoriesView.vue`, `BrowseToolsView.vue`. Removed old `BrowseView.vue` placeholder. Styling: pure CSS with HuleEdu design tokens (ADR-0029), scoped styles, responsive mobile layout. Also fixed: router uses `createWebHistory("/")`; Vite proxy excludes `/static/spa` so SPA is served by Vite dev server. Verified: `pdm run lint`, `pdm run typecheck`, `pnpm -C frontend --filter @skriptoteket/spa typecheck`. Live Playwright test: auth redirect, login, profession list (5), category list (4), tools/curated apps view all working.
- ST-11-07 (tool run + results, SPA): Added SPA-friendly tool-run API + UI views. Backend: `src/skriptoteket/web/api/v1/tools.py` (new: `GET /api/v1/tools/{slug}` + `POST /api/v1/tools/{slug}/run` multipart), `src/skriptoteket/web/router.py` (included tools router), `src/skriptoteket/application/scripting/interactive_tools.py` + `handlers/get_tool_run.py` (include `error_summary` in `RunDetails`). Frontend: new typed output components `frontend/apps/skriptoteket/src/components/ui-outputs/*` + typed next_action components `frontend/apps/skriptoteket/src/components/ui-actions/*`; new views `frontend/apps/skriptoteket/src/views/ToolRunFormView.vue` + `ToolRunResultView.vue`; routes in `frontend/apps/skriptoteket/src/router/routes.ts`; browse links updated in `frontend/apps/skriptoteket/src/views/BrowseToolsView.vue`. Types regenerated: `pdm run fe-gen-api-types`. Verified: `pdm run lint`, `pdm run typecheck`, `pdm run docs-validate`, `pnpm -C frontend --filter @skriptoteket/spa typecheck|lint|build`.
