...
- `ST-29-09` / `PR-0177` are now in progress locally: `frontend/apps/skriptoteket/src/views/apps/components/PlannerSeatingWorkspaceToolbar.vue` drops the redundant active-rules pill, `RoomTemplateEditorSidebar.vue` now gives explicit selected-tool feedback consistent with the rules toolbar, grouping/seating student bars render shared active-rule markers through `PlannerStudentRuleMarkers.vue`, and grouped students keep those markers via `PlannerGroupingWorkspacePane.vue`, `GroupBoard.vue`, and `GroupCard.vue`; docs now track the slice in `docs/backlog/stories/story-29-09-klassrumskartan-rule-visibility-and-tool-feedback-continuity.md` and `docs/backlog/prs/pr-0177-st-29-09-rule-visibility-and-tool-feedback-continuity.md`
## Verification
