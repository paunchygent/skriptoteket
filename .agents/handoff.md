...
- Completed: `ST-24-01` implemented locally across `PR-0079` to `PR-0081`; `ST-24-05` implemented locally across `PR-0082` to `PR-0085`; `PR-0086` and `PR-0087` implemented locally for `ST-24-02`.

## Current Session (2026-03-21)

- Klassrumskartan fundamentals rollback / UX repair:
  - `frontend/apps/skriptoteket/src/views/apps/ClassroomPlannerView.vue`, `frontend/apps/skriptoteket/src/views/apps/components/PlannerSelectionGate.vue`, `frontend/apps/skriptoteket/src/views/apps/components/PlannerClassWorkspace.vue`, `frontend/apps/skriptoteket/src/views/apps/useClassroomState.ts`, and `frontend/apps/skriptoteket/src/views/apps/classroomPlannerTypes.ts`: `PR-0087` is now implemented locally; the app is class-first at the root, `resume most recent work` stays on the landing screen, class selection opens a dedicated class workspace fed by the `PR-0086` summary contract, and the reusable Klassrumskartan Playwright smoke has been updated to follow the new landing -> class workspace -> seating path.
...
- 2026-03-21: `pnpm -C frontend --filter @skriptoteket/spa exec vitest run src/views/apps/ClassroomPlannerView.spec.ts src/views/apps/components/PlannerSelectionGate.spec.ts src/views/apps/components/PlannerClassWorkspace.spec.ts`; `pnpm -C frontend --filter @skriptoteket/spa exec vue-tsc --noEmit`; `pnpm -C frontend --filter @skriptoteket/spa build`; `pnpm -C frontend --filter @skriptoteket/spa exec eslint src/views/apps/ClassroomPlannerView.vue src/views/apps/useClassroomState.ts src/views/apps/classroomPlannerTypes.ts src/views/apps/components/PlannerSelectionGate.vue src/views/apps/components/PlannerClassWorkspace.vue src/views/apps/ClassroomPlannerView.spec.ts src/views/apps/components/PlannerSelectionGate.spec.ts src/views/apps/components/PlannerClassWorkspace.spec.ts`; `pdm run ruff check scripts/playwright_classroom_planner_smoke.py`; `pdm run python -m scripts.playwright_classroom_planner_smoke --base-url http://127.0.0.1:5173` after `PR-0087` class-first landing/state-machine changes (PASSED). Verified:
  - landing shows the top-level resumable CTA before class selection
  - selecting a class opens the class workspace instead of jumping straight into the planner
  - choosing a classroom inside the class workspace still opens the live seating planner
...
- Continue `ST-24-02` through the planned `PR-0088` to `PR-0089` slice chain, beginning with `PR-0088` task entry and planner return semantics.
