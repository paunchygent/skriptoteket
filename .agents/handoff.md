...
- EPIC-02 local password reset is now implemented locally against the approved docs slice: backend adds `password_reset_tokens`, hashed token lookup, normalized-email cooldowns, explicit `forgot-password`/`reset-password` contracts, and bulk session revocation through `src/skriptoteket/application/identity/handlers/request_password_reset.py`, `src/skriptoteket/application/identity/handlers/reset_password.py`, `src/skriptoteket/infrastructure/repositories/password_reset_token_repository.py`, `src/skriptoteket/infrastructure/repositories/session_repository.py`, `src/skriptoteket/web/api/v1/auth.py`, and `migrations/versions/8f3d2c1b4a6e_add_password_reset_tokens.py`; frontend adds `/forgot-password` and `/reset-password`, fixes post-register auth drift, and exposes the login-modal recovery entry point through `frontend/apps/skriptoteket/src/views/ForgotPasswordView.vue`, `frontend/apps/skriptoteket/src/views/ResetPasswordView.vue`, `frontend/apps/skriptoteket/src/components/auth/LoginModal.vue`, `frontend/apps/skriptoteket/src/stores/auth.ts`, and regenerated `frontend/apps/skriptoteket/src/api/openapi.d.ts`; `ST-02-10` / `PR-0174` are now in progress to harden recovery-email rendering, remove the broken verification-email image asset, and expose resend-verification from forgot-password plus `EMAIL_NOT_VERIFIED` login failures; runtime note: the local Docker `web` service briefly looked broken after migration because `uvicorn --reload` plus bind-mounted polling kept reloading on host-side file mtime churn, but the DB itself was current at `8f3d2c1b4a6e` and the service settled healthy once file changes stopped
- Local onboarding/personalization follow-up is now implemented locally: default `classroom.group-seating-studio` favorites now backfill through `migrations/versions/a1e4d6c8b2f0_backfill_default_klassrumskartan_bookmark.py`; the landing page and registration flow are updated in `frontend/apps/skriptoteket/src/views/HomeView.vue`, `frontend/apps/skriptoteket/src/views/RegisterView.vue`, `frontend/apps/skriptoteket/src/composables/auth/useRegistrationValidation.ts`, and `frontend/apps/skriptoteket/src/components/auth/AuthPasswordField.vue`; successful registration now stays on `/register` with explicit verify-email plus `noreply@hule.education`/skräppost guidance; `scripts/playwright_ui_smoke.py` now verifies the current landing copy, desktop CTA width parity, and protected-route login instead of the brittle `/login` assumption
...
- 2026-03-27 smart-assignment UI docs refinement:
  ...
  - added `ST-27-07` plus `PR-0155` so `Regler` is the dedicated rule-editing workspace with `Planeringskarta` / `Sittschema`, while `Sittplatser` / `Grupper` keep compact summaries and a settings-link affordance near `Smart`
...
## Next Steps
- Next checkpoints are `PR-0176` under `ST-02-10` to remediate the ruthless review findings on resend cooldown semantics and handoff reproducibility, any follow-up continuity fixes that emerge once local editor fixtures exist for a live editor-shell audit, the continuing planner redesign lane in `ST-29-03` and later `ST-29-06`, and `ST-02-09` before any multi-instance auth rollout.
