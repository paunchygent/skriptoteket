...
3. Apply the migration to the dev database once tests pass:
   ...
   - `pdm run db-upgrade`
   - If the DB volume was initialized with different credentials, reset via `pdm run dev-stack db-reset`.
...
- `pdm run dev-stack start`, `pdm run dev-stack build-start`, `pdm run dev-stack rebuild`, and `pdm run dev-stack db-reset`
...
- If Docker dev still reports impossible schema drift after auto-upgrade,
  ...
  teammate/staging DB. Use `pdm run dev-stack db-reset` only for disposable local
  cleanup.
