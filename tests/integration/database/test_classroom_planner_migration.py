...
async def test_classroom_planner_migration_idempotency(postgres_container):
    """Verifies that the classroom planner migration is idempotent and handles head re-upgrade."""
    # Arrange
    ...
    # Target revision for draft refactor
    target_revision = "4f5605f8be18"
    # The revision before any classroom planner tables
    ...
    try:
        ...
        assert "classroom_planner_group_assignments" in tables
        assert "classroom_planner_seat_assignments" in tables

        # 2. Upgrade to head again (should be no-op)
        ...
        assert "classroom_planner_plan_drafts" not in tables

        # 4. Re-upgrade to target
        run_alembic_cmd(command.upgrade, target_revision)
