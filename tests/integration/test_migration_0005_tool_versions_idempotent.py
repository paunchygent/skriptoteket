...
async def _smoke_schema(*, engine: AsyncEngine) -> None:
    async with engine.connect() as conn:
        ...
        await conn.execute(text("SELECT COUNT(*) FROM tool_runs"))
        await conn.execute(
            text(
                "SELECT tool_id, version_id, context, requested_by_user_id, status, "
                "started_at, finished_at, workdir_path, input_filename, input_size_bytes, "
                "html_output, stdout, stderr, artifacts_manifest, error_summary "
                "FROM tool_runs LIMIT 0"
            )
        )

        await conn.execute(text("SELECT active_version_id FROM tools LIMIT 0"))
...
@pytest.mark.docker
def test_migration_0005_tool_versions_is_idempotent(monkeypatch: pytest.MonkeyPatch) -> None:
