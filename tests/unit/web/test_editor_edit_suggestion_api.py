...
from skriptoteket.web.api.v1.editor import edits as edits_api
...
class EditorEditSuggestionApiProvider(Provider):
    def __init__(
        self,
        *,
        settings: Settings,
    ...
    @provide(scope=Scope.REQUEST)
    def edit_suggestion_handler(self) -> EditSuggestionHandlerProtocol:
        return cast(EditSuggestionHandlerProtocol, self._handler)
...
def handler() -> AsyncMock:
    return AsyncMock(spec=EditSuggestionHandlerProtocol)
...
def app(
    ...
    app.include_router(edits_api.router, prefix="/api/v1/editor", tags=["editor"])

    container = make_async_container(
        EditorEditSuggestionApiProvider(
            settings=settings,
            clock=clock,
...
@pytest.mark.asyncio
async def test_edit_suggestion_requires_auth(client: httpx.AsyncClient) -> None:
...
@pytest.mark.asyncio
async def test_edit_suggestion_requires_csrf(
...
@pytest.mark.asyncio
async def test_edit_suggestion_success(
    client: httpx.AsyncClient,
    settings: Settings,
    current_user_provider: AsyncMock,
    sessions: AsyncMock,
    ...
    current_user_provider.get_current_user.return_value = user
    sessions.get_by_id.return_value = session
    handler.handle.return_value = EditSuggestionResult(suggestion="return 2\n", enabled=True)
    ...
    assert response.json() == {"suggestion": "return 2\n", "enabled": True}


@pytest.mark.asyncio
async def test_edit_suggestion_eval_headers_require_admin(
    client: httpx.AsyncClient,
    settings: Settings,
    current_user_provider: AsyncMock,
    sessions: AsyncMock,
...
@pytest.mark.asyncio
async def test_edit_suggestion_includes_eval_headers_for_superuser(
    client: httpx.AsyncClient,
    settings: Settings,
    current_user_provider: AsyncMock,
    sessions: AsyncMock,
    ...
    current_user_provider.get_current_user.return_value = user
    sessions.get_by_id.return_value = session
    handler.handle.return_value = EditSuggestionResult(
        suggestion="return 2\n",
        enabled=True,
        eval_meta=PromptEvalMeta(
            template_id="edit_suggestion_v1",
            outcome="ok",
            system_prompt_chars=321,
            instruction_chars=18,
            selection_chars=8,
            prefix_chars=12,
            suffix_chars=0,
        ),
    ...
    assert response.status_code == 200
    assert response.headers["X-Skriptoteket-Eval-Template-Id"] == "edit_suggestion_v1"
    assert response.headers["X-Skriptoteket-Eval-Outcome"] == "ok"
    assert response.headers["X-Skriptoteket-Eval-System-Prompt-Chars"] == "321"
    assert response.headers["X-Skriptoteket-Eval-Instruction-Chars"] == "18"
    assert response.headers["X-Skriptoteket-Eval-Selection-Chars"] == "8"
