...
def test_prune_llm_captures_root_deletes_old_capture_dirs(tmp_path) -> None:
    now = datetime.now(timezone.utc)
    kind_root = tmp_path / "llm-captures" / "chat_ops_response"
    kind_root.mkdir(parents=True)

    old_dir = kind_root / str(uuid4())
    new_dir = kind_root / str(uuid4())
    old_dir.mkdir()
    new_dir.mkdir()
    ...
    deleted = prune_llm_captures_root(artifacts_root=tmp_path, retention_days=7, now=now)
