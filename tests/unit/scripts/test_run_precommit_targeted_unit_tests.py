...
def test_main_runs_non_excluded_unit_tests(monkeypatch, tmp_path: Path, capsys) -> None:
    ...
    assert subprocess_calls == [
        (
            ["pdm", "run", "test-parallel", "-q", "tests/unit/domain/test_fast_suite.py"],
            tmp_path.resolve(),
            False,
        )
    ]
...
def test_main_filters_out_excluded_inferred_tests_and_runs_fast_targets(
    ...
    assert result == 0
    assert subprocess_calls == [
        ["pdm", "run", "test-parallel", "-q", "tests/unit/domain/test_planner.py"]
    ]
...
def test_main_falls_back_to_tests_unit_when_no_mirrored_tests_exist(
    ...
    assert result == 0
    assert subprocess_calls == [["pdm", "run", "test-parallel", "-q", "tests/unit"]]
