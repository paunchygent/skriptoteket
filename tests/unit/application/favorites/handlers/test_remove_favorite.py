...
from skriptoteket.application.favorites.commands import RemoveFavoriteCommand
...
@pytest.mark.asyncio
async def test_remove_favorite_removes_published_tool(now: datetime) -> None:
    ...
    result = await handler.handle(
        actor=actor,
        command=RemoveFavoriteCommand(catalog_item_id=tool.id),
    )

    assert result.id == tool.id
    assert result.is_favorite is False
    favorites.remove_tool.assert_awaited_once_with(user_id=actor.id, tool_id=tool.id)


@pytest.mark.asyncio
async def test_remove_favorite_removes_curated_app_when_allowed(now: datetime) -> None:
    ...
    result = await handler.handle(
        actor=actor,
        command=RemoveFavoriteCommand(catalog_item_id=app.tool_id),
    )

    assert result.id == app.tool_id
    assert result.is_favorite is False
    favorites.remove_app.assert_awaited_once_with(user_id=actor.id, app_id=app.app_id)


@pytest.mark.asyncio
async def test_remove_favorite_raises_when_unpublished_tool(now: datetime) -> None:
    actor = make_user(role=Role.USER)
    ...
    favorites.remove_tool.assert_not_awaited()


@pytest.mark.asyncio
async def test_remove_favorite_raises_when_curated_app_not_allowed() -> None:
    actor = make_user(role=Role.USER)
    app_id = "demo.admin"
    app = CuratedAppDefinition(
        app_id=app_id,
    ...
    assert exc_info.value.code is ErrorCode.NOT_FOUND
    favorites.remove_app.assert_not_awaited()
