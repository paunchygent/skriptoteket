...
from skriptoteket.application.catalog.commands import RemoveMaintainerCommand
...
@pytest.mark.unit
@pytest.mark.asyncio
async def test_remove_maintainer_requires_admin(now: datetime) -> None:
...
@pytest.mark.unit
@pytest.mark.asyncio
async def test_remove_maintainer_rejects_missing_tool(now: datetime) -> None:
    ...
    with pytest.raises(DomainError) as exc_info:
        await handler.handle(
            actor=actor,
            command=RemoveMaintainerCommand(tool_id=tool_id, user_id=target.id),
        )
...
@pytest.mark.unit
@pytest.mark.asyncio
async def test_remove_maintainer_rejects_missing_user(now: datetime) -> None:
    actor = make_user(role=Role.ADMIN)
    tool = make_tool(now=now)
    user_id = uuid4()
    ...
    with pytest.raises(DomainError) as exc_info:
        await handler.handle(
            actor=actor,
            command=RemoveMaintainerCommand(tool_id=tool.id, user_id=user_id),
        )
...
@pytest.mark.unit
@pytest.mark.asyncio
async def test_remove_maintainer_rejects_non_maintainer(now: datetime) -> None:
    actor = make_user(role=Role.ADMIN)
    tool = make_tool(now=now)
    target = make_user(role=Role.CONTRIBUTOR)
...
async def test_remove_maintainer_removes_and_logs_audit(now: datetime) -> None:
    ...
    result = await handler.handle(
        ...
        command=RemoveMaintainerCommand(
            tool_id=tool.id, user_id=target.id, reason="No longer needed"
        ),
    )
    ...
    maintainers.remove_maintainer.assert_awaited_once_with(tool_id=tool.id, user_id=target.id)
